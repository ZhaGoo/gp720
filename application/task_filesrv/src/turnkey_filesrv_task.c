...
#define FILESRV_FS_OPERATION    0
#define FILESRV_NVRAM_OPERATION 1
...
static void FileSrvLDWAudioAppRead(P_TK_FILE_SERVICE_STRUCT para);
...
*/
			case MSG_FILESRV_NVRAM_AUDIO_APP_READ:
				...
				break;
			case MSG_FILESRV_LDW_AUDIO_APP_READ:
				FileSrvLDWAudioAppRead((P_TK_FILE_SERVICE_STRUCT)para);				
				break;
/*
	...
	while(1)
	{
		...
		switch(msg_id)
		{
			case MSG_FILESRV_SCAN_CONTINUE:
				...
				{
					msgQSend(fs_scan_msg_q_id, MSG_FILESRV_SCAN_CONTINUE, NULL, 0, MSG_PRI_NORMAL);
//					msgQSend(ApQ, MSG_APQ_UPDATE_MP3_TOTAL_INDEX, NULL, NULL, MSG_PRI_NORMAL);
				} else {
					msgQSend(ApQ, MSG_APQ_STORAGE_SCAN_DONE, NULL, NULL, MSG_PRI_NORMAL);
				}
				break;
			default:
				break;
...
static void FileUserDefineRead(P_TK_FILE_SERVICE_STRUCT para)	// added by Bruce, 2008/10/27
{
	...
	if(audio_move_data != NULL)
		read_cnt = audio_move_data(para->buf_addr, para->buf_size);
...
static void FileSrvNVRAMAudioAppRead(P_TK_FILE_SERVICE_STRUCT para)
{
	...
	ret = nv_read(para->fd, para->buf_addr, para->buf_size);
	if (ret == 0) {
...
static void FileSrvLDWAudioAppRead(P_TK_FILE_SERVICE_STRUCT para)
{
	...
	if ( (LDW[para->fd].cur_pos+para->buf_size)<LDW[para->fd].length) {
		read_cnt = para->buf_size;
	}
	else {
		read_cnt = LDW[para->fd].length - LDW[para->fd].cur_pos;
	}

	// 注意：bitstream buffer 的起始和結束要知道，否則 memcpy 會超出bitstream buffer 的邊界
	memcpy((void*)para->buf_addr, (void *)(LDW[para->fd].start_addr+LDW[para->fd].cur_pos), read_cnt);

	LDW[para->fd].cur_pos += read_cnt;
	if (LDW[para->fd].cur_pos>=LDW[para->fd].length) {
		LDW[para->fd].cur_pos = 0;
	}
...
//Return:
//=======================================================
INT32S ScanFileWait(struct STFileNodeInfo *pstFNodeInfo, INT32S index)
{
	if( (pstFNodeInfo->flag == 1) && (index > pstFNodeInfo->MaxFileNum - 1)) {
		return -2;
	}
	else if( (pstFNodeInfo->flag == 1) || 				/* file is search complete */
