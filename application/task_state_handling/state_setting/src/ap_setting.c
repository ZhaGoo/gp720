...
void ap_setting_page_draw(INT32U state,INT32U state1, INT8U *tag)
{
	...
	if(MODE_KEY_flag == 0){
		...
		icon.pos_x = 8+5;
		icon.pos_y = ICON_SELECT_BAR_START_Y + (*tag - ((*tag/4)*4))*40;
...
void ap_USB_setting_page_draw(INT32U state,INT32U state1, INT8U *tag, INT32U buff_addr)
{
	...
	ap_setting_show_GPZP_file((INT8U*)"BACKGROUND.GPZP", (INT16U *)buff_addr, &icon, SETTING_ICON_NORMAL_DRAW);
	...
	icon.pos_y = 10;
	ap_setting_icon_draw((INT16U *)buff_addr, icon_base_setting, &icon, SETTING_ICON_BLUE1_COLOR);
...
//	INT16U String_pos_y;
	...
	if(setup_date_time[6] == 0)	//yy/mm/dd
	{
		if(tag ==0)
		{
			...
			cursor_icon.pos_x = Icon_pos_x+((ICON_YEAR_SELECTBAR_WIDTH-16)>>1);
			...
			cursor_icon.pos_x = Icon_pos_x+((ICON_DAY_SELECTBAR_WIDTH-16)>>1);
...
void ap_setting_page_page_number(INT8U *tag,INT8U flag)
{
	...
	if(flag){
		type = setting_item.item_max/4;
		if(setting_item.item_max>(type*4))
			type += 1;
		temp = type+0x30;
		page_num[2] = temp;
		...
		ascii_str.pos_y = ICON_BACKGROUND_MODE_START_Y+5;
...
void ap_setting_browse_set(void)
{
	setting_item.item_start = STR_DELETE;
	setting_item.item_max = 2;
	state_str = STR_PLAYBACK;
}
...
//	t_STRING_TABLE_STRUCT str_res;
	...
	str->pos_x = icon.pos_x+icon.icon_w+5;
	str->pos_y = icon.pos_y+7;
	str->str_idx = ((*tag/4)*4) + setting_item.item_start;
	...
	switch (str->str_idx) {
		...
		case STR_DATE_STAMP:
			...
			if (type == STRING_DRAW) {
				...
				#if MENU_ITEM_PRINTF_DEBUG_ENABLE
				DBG_PRINT("STR_DATE_STAMP:%d\r\n",str->font_color);
				#endif
			}
			break;
		case STR_GSENSOR:
			
			curr_tag = ap_state_config_G_sensor_get();
			if (type == STRING_DRAW) {
				...
				ap_state_config_G_sensor_set(str->font_color);
				#if MENU_ITEM_PRINTF_DEBUG_ENABLE
				DBG_PRINT("STR_GSENSOR:%d\r\n",str->font_color);
				#endif
			}
			break;
...
//				str->str_idx = STR_NEXT_MENU;
				...
				ap_state_config_LDW_set(g_setting_LDW_choice,g_setting_LDW_value);
				curr_tag = g_setting_LDW_choice;
				g_setting_LDW_flag = 0;
				setting_item.sub_item_max = 6;
				setting_item.sub_item_start = STR_LDW_ON_OFF;
				setting_item.stage = STR_LDW;
			} else if (type == SECOND_SUB_MENU_DRAW) {
				g_setting_LDW_flag = 1;
				if(g_setting_LDW_choice == LDW_ON_OFF){
					curr_tag = ap_state_config_LDW_get(LDW_ON_OFF);
					setting_item.sub_item_max = 2;
					setting_item.sub_item_start = STR_OFF1;
				}else if(g_setting_LDW_choice == LDW_CAR_TYPE){
					curr_tag = ap_state_config_LDW_get(LDW_CAR_TYPE);
					setting_item.sub_item_max = 3;
					setting_item.sub_item_start = STR_LDW_CAR;
				}else if(g_setting_LDW_choice == LDW_SENSITIVITY){
					curr_tag = ap_state_config_LDW_get(LDW_SENSITIVITY);
					setting_item.sub_item_max = 3;
					setting_item.sub_item_start = STR_LDW_LOW;
				}else if(g_setting_LDW_choice == LDW_AREA_CHOICE){
					curr_tag = ap_state_config_LDW_get(LDW_AREA_CHOICE);
					setting_item.sub_item_max = 2;
					setting_item.sub_item_start = STR_LDW_CHINA;
				}else if(g_setting_LDW_choice == LDW_START_SPEED){
					curr_tag = ap_state_config_LDW_get(LDW_START_SPEED);
					setting_item.sub_item_max = 3;
					setting_item.sub_item_start = STR_LDW_HIGH_SPEED;
				}else if(g_setting_LDW_choice == LDW_ONOFF_SOUND){
					curr_tag = ap_state_config_LDW_get(LDW_ONOFF_SOUND);
					setting_item.sub_item_max = 2;
					setting_item.sub_item_start = STR_OFF1;
					
					
				}
				g_setting_LDW_value = curr_tag;
			} else if (type == SUB_MENU_MOVE) {
			} else if (type == FUNCTION_ACTIVE) {
				#if MENU_ITEM_PRINTF_DEBUG_ENABLE
				DBG_PRINT("STR_LDW:%d\r\n",str->font_color);
...
//				ap_setting_sensor_sccb_cmd_set((INT8U *) ap_setting_EV_data, str->font_color, sizeof(ap_setting_EV_data)/setting_item.sub_item_max);
		...
		case STR_QUICK_REVIEW:
			...
			if (type == STRING_DRAW) {
				str->str_idx = curr_tag + STR_OFF4;
			} else if (type == SUB_MENU_DRAW) {
				setting_item.sub_item_max = 3;
				setting_item.sub_item_start = STR_OFF4;
			} else if (type == FUNCTION_ACTIVE) {
...
*/
		...
		case STR_CAPTURE_DATE_STAMP:
			...
			if (type == STRING_DRAW) {
				...
				#if MENU_ITEM_PRINTF_DEBUG_ENABLE
				DBG_PRINT("STR_CAPTURE_DATE_STAMP:%d\r\n",str->font_color);
				#endif
			}
			break;
...
/*		case STR_DELAY_POWER_ON:
			curr_tag = ap_state_config_delay_power_on_get();
			if (type == STRING_DRAW) {
				str->str_idx = curr_tag + STR_OFF92;


			} else if (type == SUB_MENU_DRAW) {
				setting_item.sub_item_max = 4;
				setting_item.sub_item_start = STR_OFF92;
			} else if (type == FUNCTION_ACTIVE) {
				ap_state_config_delay_power_on_set(str->font_color);
			}
			break;
*/
		case STR_BEEP_SOUND:
...
void ap_setting_no_sdc_show(void)
{
	...
	icon.pos_x = (320-216)/2;
	icon.pos_y = (240-144)/2;
	icon.transparent = TRANSPARENT_COLOR;
	icon.icon_w = 216;
	icon.icon_h = 144;
