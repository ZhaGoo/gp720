...
#define DENOISE_SELECT 		BOTH_DENOISE_AUTO
...
static INT32U denoise_level = DENOISE_DISABLE;
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
#define USE_INDEPEDENT_TASK_FOR_AE		0

#if USE_INDEPEDENT_TASK_FOR_AE

#define AE_SERVICE_TASK_STACK_SIZE		256
#define AE_SERVICE_TASK_QUEUE_MAX		64

#define	MSG_DO_SENSOR_AE		0x5a5a

static INT32U AeServiceTaskStack[AE_SERVICE_TASK_STACK_SIZE];
static void *ae_service_task_q_stack[AE_SERVICE_TASK_QUEUE_MAX];
OS_EVENT *AeServiceTaskQ = NULL;

static void ae_service_task_create(void);
static void ae_service_task(void *para);
...
extern	INT8U gsensor_lock_flag;
...
void gp_cdsp_set_new_denoise(INT32U sharpness)
{
	...
	NewDenoise.ndedgeen = DISABLE;
	NewDenoise.ndeluten = DISABLE;
...
static void gp_cdsp_handle_eof(void)
{
	...
	{
	  #if USE_INDEPEDENT_TASK_FOR_AE
		if(AeServiceTaskQ != NULL)
		{
			OSQPost(AeServiceTaskQ, (void *)MSG_DO_SENSOR_AE);
		}
	  #else
		if(my_AVIEncodeApQ)
		{
			OSQPostFront(my_AVIEncodeApQ, (void *) MSG_CDSP_AE_SHUTTER);
		}
	  #endif
	}
	...
	{						
		...
		else
		{
			...
			hwCdsp_SetEdgeAmpga(edge_level[0], 0);
			CdspDev->ae_awb_flag &= (~CDSP_SAT_SWITCH);
		}
	}
	...
	while (i < drop_frame)
	{
		...
		if(cdsp_eof_occur_flag)
		{
			...
			if((t - time_begin) > 50) { //500ms
				DBG_PRINT("drop frame wait error!\r\n");
				break;
			}
		}
		#endif
			...
			{			
				...
				/*		DBG_PRINT("\r\n\r\n\r\nAE win output:");
					...
					if(seInfo.ae_ev_idx != 0)
					{
						...
						//DBG_PRINT("\r\n0 S_Digital Gain: %x, %x, %x\r\n", seInfo.digital_gain, seInfo.max_digital_gain, seInfo.min_digital_gain);
						//DBG_PRINT("0      S_Analog Gain: %x, %x, %x\r\n", seInfo.analog_gain, seInfo.max_analog_gain, seInfo.min_analog_gain);
						//DBG_PRINT("0	  S_Time: %x, %x, %x\r\n", seInfo.time, seInfo.max_time, seInfo.min_time);
					...
					*/
					...
					if(ret != AWB_RET)
					{
						...
						if(awbmode == AWB_AUTO_CVR || awbmode == AWB_AUTO_CVR_DAYLIGHT || awbmode == AWB_AUTO_CVR_NIGHT)
						{
							...
							//DBG_PRINT("rgain = 0x%x, bgain = 0x%x\r\n", wb_gain.rgain, wb_gain.bgain);
...
void ae_service_task_create(void)
{
	AeServiceTaskQ = OSQCreate(ae_service_task_q_stack, AE_SERVICE_TASK_QUEUE_MAX);
	OSTaskCreate(ae_service_task, (void *) 0, &AeServiceTaskStack[AE_SERVICE_TASK_STACK_SIZE - 1], AE_SERVICE_TASK_PRIORITY);
}


void ae_service_task(void *para)
{
	INT32U msg_id;
	INT8U err;

	while(1) {
		msg_id = (INT32U) OSQPend(AeServiceTaskQ, 0, &err);
		if((!msg_id) || (err != OS_NO_ERR)) {
        	continue;
        }

		switch (msg_id) {
			case MSG_DO_SENSOR_AE:
				if (CdspDev->ae_ret == AE_CHANGE) 
				{
					if(gsensor_lock_flag == 0)
					{
						gsensor_lock_flag = 1;
					#if 0
...
void gp_isp_start(INT32U dummy_addr,INT32U gpSENSOR_WIDTH, INT32U gpSENSOR_HEIGHT)	
{
	...
	IspHybridRaw.DenoiseEnable = DISABLE;
...
//#endif
	...
	hwCdsp_SetRawDataFormat(2);			//BGGR
	#if SENSOR_FLIP
	hwFront_SetMipiFrameSize(0xB, 1, gpSENSOR_WIDTH, gpSENSOR_HEIGHT);	//BGGR
	#else
	hwFront_SetMipiFrameSize(0xC, 0, gpSENSOR_WIDTH, gpSENSOR_HEIGHT);
	#endif
	sensor_gc1004_init(gpSENSOR_WIDTH, gpSENSOR_HEIGHT);
#else	
	DBG_PRINT("NO Sensor Init !\r\n");
#endif
//drv_msec_wait(500); //wait 1 is 1ms for CLKO stable
	//cdsp output image format
	hwCdsp_SetOutYUVFmt(YVYU);
#if CDSP_DEBUG == 0
	/*AE, AWB Init & Enable*/
...
INT8S gp_cdsp_set_exposure_time(void)
{
	if (CdspDev->ae_ret == AE_CHANGE) 
	{
		if(gsensor_lock_flag == 0)
