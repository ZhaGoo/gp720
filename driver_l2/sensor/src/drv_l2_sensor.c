...
extern INT32S gc1004_switch_pclk(INT8U flag);
#endif
...
void sensor_start(INT32U frameAddrs,INT32U motionAddrs)
{
	...
	sensor_frame_rate = 30; // defatue sensor frame rate
	sensor_Max_frame_rate = 30;
...
{
	if(fpsValue > sensor_Max_frame_rate)
	{
		fpsValue = sensor_Max_frame_rate;
	}

	if(fpsValue == sensor_frame_rate)
	{
		return;
	}
	...
	if(fpsValue == 25)
	{
		...
		#elif ((USE_SENSOR_NAME == SENSOR_GC1004) && (sensor_format == GC1004_RAW))
		sccb_write(0x78, 0x08, 0x90);
 ...
 *	sensor_get_fps:
 */
INT32S sensor_switch_pclk(INT8U flag)
{
  #if(USE_SENSOR_NAME == SENSOR_GC1004)
	gc1004_switch_pclk(flag);
  #endif
	return 0;
}
...
sensor_apis_ops* sensor_attach(void)
{
	...
	Sensor_apis_ops.switch_pclk = sensor_switch_pclk;
