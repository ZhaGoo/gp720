...
void (*cdsp_do_refresh_callback)(void);
#endif
...
void MipiIF_Setting( unsigned char mipi_type, unsigned int width, unsigned int height)	/* 0: Raw 1:YUV */
{
	...
	(*(volatile unsigned *)0xD0F0000c)  = 0x00010807; //P_ECC_ORDER
...
//+++
#if USE_BYPASS_SCALER1_PATH
void cdsp_do_refresh_isr_register(void (*user_isr)(void))
{
	cdsp_do_refresh_callback = user_isr;
}
#endif
//---
...
void cdsp_isr(void)
{
	if(R_CDSP_GINT & CDSP_INT_BIT) //cdsp int
	{
		...
		if(R_CDSP_INT & R_CDSP_INTEN & CDSP_AEWIN_SEND)
		{
			...
			if((cdsp_overflow_occur_flag == 0) && (PreviewIsStopped == 0) &&(cdsp_ae_callback != 0))
	    	{
				(*cdsp_ae_callback)();
			}
		...
		{
			R_CDSP_INT |= CDSP_AWBWIN_UPDATE;
			if((cdsp_overflow_occur_flag == 0) && (PreviewIsStopped == 0) && (cdsp_awb_callback != 0))
	    	{
				(*cdsp_awb_callback)();
			}
		}
		...
		if(R_CDSP_INT & R_CDSP_INTEN & CDSP_EOF)
		{
			...
			else if(PreviewIsStopped)
			{
				return;
			}
			#if USE_BYPASS_SCALER1_PATH
			else if(cdsp_do_refresh_callback != 0)
			{
				(*cdsp_do_refresh_callback)();
			}
			#endif
			//---
...
)
{
	...
	if(section == 1) {		
		...
		*sumrgl = R_CDSP_SUM_RG1_L & 0x0000ffff;
		t = R_CDSP_SUM_RG1_H;
		t <<= 14;
		t >>= 14;
		*sumrgh = t;
		...
		*sumrgl = R_CDSP_SUM_RG2_L & 0x0000ffff;
		t = R_CDSP_SUM_RG2_H;
		t <<= 14;
		t >>= 14;
		*sumrgh = t;
		...
		*sumrgl = R_CDSP_SUM_RG3_L & 0x0000ffff;
		t = R_CDSP_SUM_RG3_H;
		t <<= 14;
		t >>= 14;
		*sumrgh = t;
...
)
{
	...
	if(section == 1) {
		...
		*sumbgl = R_CDSP_SUM_BG1_L & 0x0000ffff;
		t = R_CDSP_SUM_BG1_H;
		t <<= 14;
		t >>= 14;
		*sumbgh = t;
		...
		*sumbgl = R_CDSP_SUM_BG2_L & 0x0000ffff;
		t = R_CDSP_SUM_BG2_H;
		t <<= 14;
		t >>= 14;
		*sumbgh = t;
		...
		*sumbgl = R_CDSP_SUM_BG3_L & 0x0000ffff;
		t = R_CDSP_SUM_BG3_H;
		t <<= 14;
		t >>= 14;
		*sumbgh = t;
